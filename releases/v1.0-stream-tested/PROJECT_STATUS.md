...
- **Участники snapshot:** `CURRENT_ROUND_PARTICIPANTS` (`roundParticipants`); карточки = `!eliminated` (RESCUE остаётся); WAITING holder сразу; `pushNow` после gift/reset; `pushNowAfterElimination` при финальном knockout; порядок `joinOrder`.
...
Локальный HTTPS overlay для TikTok LIVE Studio (захват окна + chroma `#FF00FF`). In-game Arena HUD отключён. Адаптивная сетка 1–20 стран + мгновенный WAITING holder. Названия стран над флагами баз в мире.
